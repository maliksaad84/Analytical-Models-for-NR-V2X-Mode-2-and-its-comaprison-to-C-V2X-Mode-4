function [ deltaCOL , CBR ] = NRV2XMode2_Step2( beta , lambda , Pt , S , distance, Psen , step_dB , noise , coding , deltaPRO )

% NRV2XMode2_Step2 is a script that calculates the probability of collision 
...
%
    ...
    N = S * 4000/lambda;  % Total number of resources in 1000ms.   (NR-V2X with 60 kHz subcarrier spacing)
    ...
    Spsr = beta * sum(PSR) ;  
    Ne =  Spsr/2 + sum(max( 1 - (1:(Spsr/2))/(N-Spsr/2) , 0 )); % Number of excluded resources.
    Na = N - Ne; % Number of assignable resources.
    CBR = Ne / N; % Channel busy ratio. 
    ...
    Ce = ( R_PSR / R0 ) * (beta*Ne*R0/Spsr - Ne*Ne/N) + Ne*Ne/N; % Common excluded resources. 
    Ca = N - 2*Ne + Ce;  % Common available resources. 
    Cc = Ca * (Nc/Na)^2; % Common candidate resources.    
    ...
    for d=1:length(distance)  
        ...
        for i = 1:length(distance_int_to_rx)    
            ...
            p_s(i) = 1-(1-1/tau)*pSsensing_I_E; % Probability that transmitter and interferer do not take into account their respective transmissions before selecting a new resource.
                        
            p_sim_2(i) = p_s(i) * Cc( round( abs(distance_int_to_tx(d,i))  ) + 1 ) / (Nc*Nc) ; 

        end

        p_col = p_sim_2 .* p_int(d,:);      
